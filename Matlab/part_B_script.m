...
imshow('Figures/Part_B_Circuit_Diagram.png');
...
function [Current] = Branch_Current_Calculation(matrix, Voltages)
    ...
    for I = 1:3 % loop 3 times for each i value and element in row
        ...
        % print calculation and equation
        fprintf("Mesh %d equation: Current(Mesh %d) = Voltage(Mesh %d) * minor * cofactor / char_det \n" , I, I, I)
        fprintf("Mesh %d equation: Current(Mesh %d) = %f * %f * %d / %f \n" , I, I, Voltages(I), minor, cofactor, char_det)
        fprintf("Mesh %d equation: Current(Mesh %d) = %f Amps \n \n", I, I, Current(I)) 
    end
